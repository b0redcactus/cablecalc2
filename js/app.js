...
(function () {
  ...
  function lookupBase() {
    const opts = {
      ...
      screening:  $('f-screening').value
    };
    const v = window.CALC.lookupBaseAmpacity(opts);
    if (v != null) {
      $('f-base').value = v;
      const src = tableSourceLabel(opts.insulation, opts.voltage, opts.installation);
      $('base-source').textContent = '✓ MSZ 13207 ' + (src || '');
  ...
  // Map (insulation, voltage, installation) → the MSZ 13207 table number used for the lookup.
  function tableSourceLabel(insulation, voltage, installation) {
    if (insulation === 'PAPER65' || insulation === 'PAPER60') {
      return installation === 'soil' ? 'Tábl. 6' : null;
    }
    if (insulation === 'XLPE' && voltage !== '0.6/1') {
      return installation === 'soil' ? 'Tábl. 7.a' : 'Tábl. 7.b';
    }
    if (voltage === '0.6/1' && insulation === 'PVC')  return installation === 'soil' ? 'Tábl. 8' : 'Tábl. 9';
    if (voltage === '0.6/1' && insulation === 'XLPE') return installation === 'soil' ? 'Tábl. 10' : 'Tábl. 11';
    return null;
  }
  ...
  function calculate() {
    ...
    const screening    = $('f-screening').value;
    const parallelN    = Math.max(1, parseInt($('f-parallel').value, 10) || 1);
    const result = {
      installation, insulation, voltage, material, section, arrangement,
      base: isNaN(base) ? null : base,
      duct, screening,
      f1: null, f2: null, f3: null, f4: null,
      ductFactor: duct ? 0.85 : 1.0,
      parallelN,
      iN: null, iN_total: null,
      iLoad: null, ok: null,
    ...
    // Per-cable ampacity (Iₐ · f1·f2·duct or Iₐ · f3·f4·duct).
    // Circuit total ampacity is N × per-cable.
    if (result.base != null && result.base > 0) {
      ...
      if (fA != null && fB != null) {
        ...
        result.iN_total = result.iN * parallelN;
      }
    }

    // Optional system check — load is shared equally between N parallel cables.
    ...
    if (!isNaN(sysU) && !isNaN(sysP) && !isNaN(cosphi)) {
      ...
      if (result.iN_total != null && result.iLoad != null) {
        result.ok = (result.iN_total >= result.iLoad);
      }
    }

    // Optional voltage drop — ΔU is reduced by 1/N because the current splits across N parallel cables.
    ...
    if (!isNaN(len) && len > 0 && result.iLoad != null && !isNaN(sysU) && !isNaN(cosphi)) {
      ...
      if (R != null && X != null) {
        ...
        if (vd) {
          result.vd = vd.dU / parallelN;
          result.vdPct = vd.percent / parallelN;
        }
      }
    }
  ...
  function renderResults(r) {
    ...
    // Per-cable row — always visible. Label switches between "Ampacity" and "Ampacity / cable" depending on N.
    const perKey = (N > 1) ? 'res.iN_per' : 'res.iN';
    const perLabel = $('kv-iN').querySelector('.k');
    perLabel.setAttribute('data-i18n', perKey);
    perLabel.textContent = window.t(perKey);
    ...
    // Total circuit ampacity row — visible only when N > 1.
    const kvTotal = $('kv-iN-total');
    if (N > 1) {
      kvTotal.style.display = '';
      $('r-iN-total').textContent = (r.iN_total != null) ? fmt(r.iN_total, 1) + ' A' : NA;
    } else {
      kvTotal.style.display = 'none';
    }
    kvTotal.classList.remove('ok','bad');
    ...
    // Adequacy check colours whichever ampacity row is the operative one
    // (the per-cable row when N=1, the circuit-total row when N>1).
    const activeRow = (N > 1) ? kvTotal : $('kv-iN');
    const checkEl = $('r-check');
    if (r.ok === true) {
      checkEl.textContent = window.t('res.check.ok');
      checkEl.className = 'badge ok';
      activeRow.classList.add('ok'); activeRow.classList.remove('bad');
    } else if (r.ok === false) {
      checkEl.textContent = window.t('res.check.bad');
      checkEl.className = 'badge bad';
      activeRow.classList.add('bad'); activeRow.classList.remove('ok');
    } else {
      checkEl.textContent = window.t('res.check.na');
      checkEl.className = 'badge';
      activeRow.classList.remove('ok','bad');
    }
  ...
  function init() {
    ...
    $('f-screening').addEventListener('change', maybeAutoLookup);

    // Recalculate on any input change in calc-affecting fields
    [
      'f-material','f-section','f-arrangement','f-duct','f-parallel','f-screening',
